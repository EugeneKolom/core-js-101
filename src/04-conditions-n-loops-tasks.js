...
function isCreditCardNumber(ccn) {
  ...
  for (let i = strCcn.length - 1; i >= 0; i -= 1) {
    let currentDigit = parseInt(strCcn.charAt(i), 10);
...
function evaluateTicTacToePosition(position) {
  ...
  for (let i = 0; i < 3; i += 1) {
    if (position[i][0] && position[i][0] === position[i][1] && position[i][0] === position[i][2]) {
      return position[i][0]; // Check row for a winner
    }
    if (position[0][i] && position[0][i] === position[1][i] && position[0][i] === position[2][i]) {
      return position[0][i]; // Check column for a winner
    }
  }

  // Check diagonals for a winner
  if (position[0][0] && position[0][0] === position[1][1] && position[0][0] === position[2][2]) {
    return position[0][0]; // Check diagonal from top left to bottom right
  }
  if (position[0][2] && position[0][2] === position[1][1] && position[0][2] === position[2][0]) {
    return position[0][2]; // Check diagonal from top right to bottom left
  }

  return undefined; // If no winner, return undefined
}
